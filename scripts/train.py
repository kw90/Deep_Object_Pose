...
class DopeNetwork(nn.Module):
    ...
    def create_stage(in_channels, out_channels, first=False):
        ...
        model.add_module(
            str(i),
            nn.Conv2d(mid_channels, final_channels, kernel_size=1, stride=1),
        )
        i += 1
        ...
        model.add_module(
            str(i),
            nn.Conv2d(final_channels, out_channels, kernel_size=1, stride=1),
        )
        i += 1
...
def loadjson(path, objectsofinterest, img):
    ...
    for i_line in range(len(data["objects"])):
        ...
        if (
            not objectsofinterest is None
            and not objectsofinterest in info["class"].lower()
        ):
            continue
...
class MultipleVertexJson(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.save or self.test:
            ...
            def DrawLine(point1, point2, lineColor, lineWidth):
                if not point1 is None and not point2 is None:
                    draw.line(
                        [point1, point2], fill=lineColor, width=lineWidth
                    )
        ...
        beliefsImg = CreateBeliefMap(
            img, pointsBelief=pointsBelief, nbpoints=9, sigma=self.sigma
        )
        ...
        affinities = affinities[
            :,
            h_crop : h_crop + int(img_size[1] / 8),
...
def GenerateMapAffinity(
    img, nb_vertex, pointsInterest, objects_centroid, scale
):
    """
...
def getAfinityCenter(
    width, height, point, center, radius=7, img_affinity=None
):
    """
    Function to create the affinity maps,
    e.g., vector maps pointing toward the object center.
...
def make_grid(
    ...
    grid = tensor.new(
        3, height * ymaps + padding, width * xmaps + padding
    ).fill_(pad_value)
    k = 0
...
parser.add_argument(
    "--object", default=None, help="In the dataset which objet of interest"
)
...
parser.add_argument(
    "--net", default="", help="path to net (to continue training)"
)
...
parser.add_argument(
    "--gpuids", nargs="+", type=int, default=[0], help="GPUs to use"
)
...
parser.add_argument(
    "--sigma", default=4, help="keypoint creation size for sigma"
)
...
if opt.save:
    for i in range(2):
        ...
        save_image(
            images["img"],
            "{}/train_{}.png".format(opt.outf, str(i).zfill(5)),
            mean=normal_imgs[0],
            std=normal_imgs[1],
        )
    ...
    for batch_idx, targets in enumerate(loader):
        ...
        if train:
            if batch_idx % opt.loginterval == 0:
                print(
                    "Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.15f}".format(
                        epoch,
                        batch_idx * len(data),
                        len(loader.dataset),
        ...
        # break
        if not opt.nbupdates is None and nb_update_network > int(
            opt.nbupdates
        ):
            torch.save(
                net.state_dict(),
                "{}/net_{}.pth".format(opt.outf, opt.namefile),
            )
            break
...
for epoch in range(1, opt.epochs + 1):
    ...
    try:
        torch.save(
            net.state_dict(),
            "{}/net_{}_{}.pth".format(opt.outf, opt.namefile, epoch),
        )
    except:
        pass
